...
// Shared lesson catalog (kept local to avoid pulling practice.js into this page)
const lessonsData = [
  // ENGLISH LESSONS
  {
    id: 1,
...
// Exercises for every lesson (mirrors practice page so lesson links stay in sync)
const exercisesData = {
  1: [
    {
      id: 1,
...
// Map lesson language to locale for TTS/recognition
function getLanguageLocale(language = "english") {
  const mapping = {
    english: "en-US",
    spanish: "es-ES",
    japanese: "ja-JP",
...
function clearWaveformAnimation() {
  waveformIntervals.forEach((intervalId) => clearInterval(intervalId));
  waveformIntervals = [];
}
...
function loadLesson(lessonId) {
  ...
  renderExerciseList();
  loadCurrentExercise();
  setRecognitionLanguage(currentLesson.language);

  // Check if lesson is already completed in the backend
  fetch(`get_progress.php?lesson_id=${lessonId}`)
    .then(response => response.json())
    .then(data => {
      if (data.completed) {
        // Mark all potentially as completed or just show completion logic
        // For now, let's just create a visual indication or pre-fill completion
        // Note: This matches the user request to maintain progress on revisit
        exercises.forEach((_, index) => completedIndices.add(index));
        renderExerciseList();
      }
    })
...
// Update lesson information in the header
function updateLessonInfo() {
  ...
  if (lessonLanguage) {
    lessonLanguage.textContent = currentLesson.language.toUpperCase();
  }
  if (lessonLevel) {
    lessonLevel.textContent = `Level ${currentLesson.levelNumber}`;
...
function renderExerciseList() {
  ...
  exercisesContainer.innerHTML = exercises
    .map((exercise, index) => {
      ...
      let iconStyle = "color: #9ca3af"; // Gray

      if (index === currentExercise) {
        statusClass = "active";
        statusText = "In Progress";
        iconStyle = "color: #6366f1"; // Primary
      } else if (completedIndices.has(index)) {
        statusClass = "completed"; // Optional: add a completed class to the item for styling
        statusText = "Completed";
        iconClass = "fas fa-check-circle";
        iconStyle = "color: #10b981"; // Green
      }

      return `
      <div class="exercise-item ${statusClass}" data-exercise-index="${index}">
...
// Update exercise list visual state
function updateExerciseList() {
  document.querySelectorAll(".exercise-item").forEach((item, index) => {
    item.classList.remove("active");
    // Ensure we handle the completed class if we used it in render
    item.classList.remove("completed");

    const statusSpan = item.querySelector(".exercise-status span");
    const statusIcon = item.querySelector(".exercise-status i");

    if (index === currentExercise) {
      item.classList.add("active");
      if (statusSpan) statusSpan.textContent = "In Progress";
      if (statusIcon) {
        statusIcon.style.color = "#6366f1";
        statusIcon.className = "fas fa-circle"; // Reset to circle for active
      }
    } else if (completedIndices.has(index)) {
...
function loadCurrentExercise() {
  ...
  const currentExerciseNumber = document.getElementById(
  ...
  if (currentExerciseNumber) {
    currentExerciseNumber.textContent = currentExercise + 1;
  }
  if (totalExercises) {
...
function setupSpeechRecognition() {
  if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
    ...
    recognition.lang = getLanguageLocale(currentLesson?.language);
...
function playAudio() {
  ...
  if (!audioBtn || !waveform || !exercise) return;
  ...
  const locale = getLanguageLocale(currentLesson?.language);
...
function nextExercise() {
  ...
  if (exercise) {
    ...
    totalScore += score;
    completedIndices.add(currentExercise);
  }
...
function showCompletionModal() {
  ...
  saveProgress(currentLesson.id, averageScore);
}
